...
import { TableColumn, TableFooter } from '../../Interfaces/list-table.interface';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NgxSpinnerModule, MatTableModule, MatSortModule, AppRoutingModule, FormsModule, MatTooltipModule, MatIconModule, MatPaginatorModule],
...
export class ListTableComponent<T> implements OnInit, AfterViewInit, OnChanges {
  ...
  async getDataByService(){
    this.changesGetInputs.pipe(startWith(""), switchMap(() => {
      this._spiner.show("modal-table");
      return this.service.GetAll(this.paginator.pageSize, this.sort.active, this.sort.direction, this.paginator.pageIndex, this.getInputs);
    })).subscribe((data: any) => {
      this.setData(data)
      this._spiner.hide("modal-table");
    })
  }
  ...
  setData(data: any){
    if(data.isSuccess){
      this.setVisibleColumns();
      this.paginatorOptions.pageLenght = data.data.totalRecords;
